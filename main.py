...
shop_items = [
    {
        "id": 1,
        "name": "Photo Border [NO]",
        "price": 0,
        "border_color": None,
        "image_url": "/static/images/item1.jpg",
    },
    {
        "id": 2,
        "name": "Color Name",
        "price": 500,
        "image_url": "/static/images/item2.jpg",
    },
    {
        "id": 3,
        "name": "Remove Border Width",
        "price": 0,
        "image_url": "/static/images/item3.jpg",
    },
    {
        "id": 4,
        "name": "Border Width [NO]",
        "price": 99999,
        "image_url": "/static/images/item4.jpg",
    },
]
...
class Photo(db.Model):
    ...
    likes = db.relationship(
        "Like", backref="photo", lazy=True, cascade="all, delete-orphan"
    )
    comments = db.relationship(
        "Comment", backref="photo_ref", lazy=True, cascade="all, delete-orphan"
    )
    border_color = db.Column(
        db.String(20)
    )  # Store the border color associated with the photo
...
class User(db.Model, UserMixin):
    ...
    selected_username_color = db.Column(
        db.String(7)
    )  # Store color as a hex string (e.g., "#RRGGBB")
    ...
    trivia_score = db.Column(
        db.Integer, default=0
    )  # Add this line to store the trivia score
    has_purchased_border_width = db.Column(db.Boolean, default=False)
    selected_border_width = db.Column(
        db.Integer, default=1
    )  # Default border width is 1px
...
def get_notifications():
    ...
    return jsonify(
        [
            {"id": notification.id, "content": notification.content}
    ...
    if photo_id and comment_text:
        ...
        if photo:
            ...
            notification = Notification(
                user_id=photo.user_id, content=notification_content
            )
            db.session.add(notification)
            db.session.commit()
    ...
    if friend:
        if request.method == "POST":
            ...
            if message:
                ...
                new_message = Message(
                    sender_id=current_user.id, recipient_id=friend.id, text=message
                )
...
def send_message():
    ...
    if message_text and recipient_id:
        ...
        message = Message(
            sender_id=current_user.id, recipient_id=recipient_id, text=message_text
        )
...
def user_profile(username):
    ...
    if user:
        ...
        user_reputation = (
            user.reputation
        )  # Replace 'reputation' with the actual field in your User model
    ...
    if request.method == "POST":
        ...
        if can_give_reputation(current_user.id, receiver_user_id):
            if time_left.total_seconds() <= 0:
                ...
                return jsonify(
                    {
                        "error": "You can only give reputation once every 24 hours.",
...
def has_given_reputation(giver_user_id, receiver_user_id):
    ...
    reputation_given = ReputationGiven.query.filter_by(
        giver_user_id=giver_user_id, receiver_user_id=receiver_user_id
    ).first()
...
def mark_gave_reputation(giver_user_id, receiver_user_id):
    ...
    # Example implementation using SQLAlchemy:
    reputation_given = ReputationGiven(
        giver_user_id=giver_user_id, receiver_user_id=receiver_user_id
    )
...
def trivia_game():
    ...
    if request.method == "POST":
        ...
        if (
            feedback == "Correct!"
            and current_user.trivia_score < current_question_index + 1
        ):
...
def shop():
    ...
    return render_template(
        "shop.html", shop_items=shop_items, user_balance=user_balance
    )
...
def update_border_width():
    ...
    if current_user.has_purchased_border_width:  # Add this attribute to your User model
        if new_border_width is not None:
            ...
            return (
                jsonify({"success": False, "error": "Invalid border width value"}),
                400,
            )
    else:
        return (
            jsonify(
                {
                    "success": False,
                    "error": "You have not purchased the 'Border Width' item",
                }
            ),
            403,
        )
...
def modify_user():
        ...
        if "search_user" in request.form:
            ...
            if user_id and coins:
                ...
                if user_to_modify:
                    ...
                    db.session.commit()
                    flash(
                        f"Coins for user {user_to_modify.username} modified to {coins}",
                        "success",
                    )
                else:
            ...
            if username_to_search:
                ...
                if found_user:
                    flash(
                        f"User found: {found_user.username}, ID: {found_user.id}",
                        "success",
                    )
    ...
    return render_template(
        "admin.html", found_user=found_user, found_user_id=session.get("found_user_id")
    )
